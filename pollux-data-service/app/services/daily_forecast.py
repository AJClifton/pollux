...
class DailyForecast:
    ...
    def _load(self):
        ...
        cache_suffix = f"{self.lat}:{self.lon}"
        if cached := redis_service.cache_get("forecast:daily", cache_suffix):
            return cached
        ...
        redis_service.cache_set("forecast:daily", cache_suffix, result, redis_ttl)
        return result
